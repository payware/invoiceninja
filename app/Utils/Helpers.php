...
class Helpers
{
     ...
     * locale the document is being rendered in - and therefore also in whatever
     * `settings.translations` holds for that client, which is where payware keeps the words.
     * Anything else is returned unchanged, so every label anyone has already typed keeps
     * working, and a key with no translation falls back to the label as written rather than
     * printing a blank heading over a value.
     *
     ...
     * caption can ride on a custom field that is never printed as a value.
     */
